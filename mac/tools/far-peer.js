...
//
// CommonJS ON PURPOSE, and the FILE NAME IS PART OF THAT. The container
// saves this as join.js and runs `node join.js`, so the body has to be
// require-based -- but it was published as `far-peer.mjs`, and `.mjs`
// forces ES module scope, where `require` does not exist. Every person who
// followed the usage line below got a ReferenceError on line 14 and never a
// single packet. HANDOFF.md records the same trap once already.
//
// The extension is not decoration. It is the module system.
...
if (!ROOM) {
  console.error('usage: ROOM=<code> node far-peer.js');
  process.exit(2);
}
