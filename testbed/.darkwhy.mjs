...
for (let run = 1; run <= 3; run++) {
  ...
  for (let i = 0; i < 8 && !ready; i++) {
    await page.goto(`${BASE}/?ladder=0&lowlight=0&revive=0`, { waitUntil: 'domcontentloaded' });
  ...
  for (let i = 0; i < 16; i++) {
    ...
    m.push(await page.evaluate(() => ({ y: window.__tape?.luma?.y ?? null, f: window.__tape?.srcprobe?.frames ?? null, lk: window.__tape?.camlock?.locked ?? null })));
  }
  const tail = m.slice(-6).map((x) => x.y).filter((x) => x != null);
  const meanTail = tail.length ? (tail.reduce((a, c) => a + c, 0) / tail.length).toFixed(0) : '—';
  console.log(`  run ${run}  final luma ${String(meanTail).padStart(4)}  locked=${m[m.length-1].lk}`);
