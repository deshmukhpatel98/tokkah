...
| ringing with the app closed costs a notification | Android requires a foreground service notification. MEASURED 2026-09-03 (API 35 emulator, dumpsys notification): a channel created at MIN reads mImportance=2 mOriginalImp=1, and a fresh channel created at NONE ALSO reads mImportance=2 with the notification posted — the system promotes any foreground-service channel to LOW, so no channel setting removes it. The only route without a notification is push (FCM), which needs a Firebase project the owner would have to create | ⛔ platform, floor measured both ways |
