...
| the invite link's shape | `roomURL` (main.swift 1668): a minted 3-4-3 room is `kin.tokkah.com/<room>`, anything else is `/?r=<room>` because the short path 404s for a named room. The phone used the short path for everything. Ported to `Server.roomURL`, held by RoomUrlTest (minted, named, hyphens, almost-minted) | ✅ android.25 |
| the link pill in the waiting row | the Mac shows the WHOLE link (a person proof-reads it before a browser does) in a box as wide as the text + s6, min 180, max 70% of the window (Controls.swift 2819). The phone stripped `https://` and used a fixed 180–320 box. Now the full link, box by the Mac's rule against the screen width | ✅ android.25 |
| the sky pills under the sheet | the Mac's sheet is a panel at the right and the sentence stays in open sky beside it; the phone's sheet is the whole width and the status/warn pills sat UNDER its glass as ghosts (effect-view-resamples-siblings). Hidden while the sheet is up | ✅ android.26 |
| the empty picture is a black band | the far video is a SurfaceView — a hole in the window — and with their camera off the hole is pure black inside a #06080D ground: a rectangle in the middle of the call. The Mac hides its picture layer; a SurfaceView cannot be hidden without losing the decoder's surface, so the ground now covers it while `noPicture` is set | ✅ android.26 |
| updates meet Google Play Protect | a sideloaded APK's first install of a new version can be intercepted by Play Protect ("App scan recommended … Don't install app") — seen on the emulator taking .25. The Mac's equivalent is the one-time Gatekeeper click. Only Play distribution removes it | ⛔ platform, noted |
| a ring that lands as Kin goes behind | the poll in flight when ON_STOP hit delivered a ring to the app's own poller, which drew the card in a backgrounded window and rang on (measured: 1 of 4 test rings). An in-app offer now raises the window under the same grant the service uses | ✅ android.27 |
